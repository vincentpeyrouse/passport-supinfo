 ...
 * @api private
 */
Strategy.prototype._parseProfileExt = function(params, query) {
  var profile = {};
  ...
  profile.ranks = [];
  if (query['openid.alias3.count.alias5'] != '1') {
    for (var i=1; i<=query['openid.alias3.count.alias5'];i++) {
      profile.ranks.push(query['openid.alias3.value.alias5.'+i]);
    }
  } else {
    profile.ranks.push(query['openid.alias3.value.alias5']);
  }

  profile.fullProfSubjects = [];
  if (query['openid.alias3.count.alias6'] != '1') {
    for (var i=1; i<=query['openid.alias3.count.alias6'];i++) {
      profile.fullProfSubjects.push(query['openid.alias3.value.alias6.'+i]);
    }
  } else {
    profile.fullProfSubjects.push(query['openid.alias3.value.alias6']);
  }
  
  profile.teacherSubjects = [];
  if (query['openid.alias3.count.alias7'] != '1') {
    for (var i=1; i<=query['openid.alias3.count.alias7'];i++) {
      profile.teacherSubjects.push(query['openid.alias3.value.alias7.'+i]);
    }
  } else {
    profile.teacherSubjects.push(query['openid.alias3.value.alias7']);
  }
